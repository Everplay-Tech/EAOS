...
impl Clone for EntropyRequest {
    fn clone(&self) -> Self {
        match self {
            Self::Random { salt_len, nonce_len } => Self::Random {
                salt_len: *salt_len,
                nonce_len: *nonce_len,
            },
            Self::Provided { salt, nonce, manager } => Self::Provided {
                salt: salt.clone(),
                nonce: nonce.clone(),
...
impl std::fmt::Debug for EntropyRequest {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Random { salt_len, nonce_len } => f
                .debug_struct("Random")
                .field("salt_len", salt_len)
                .field("nonce_len", nonce_len)
                .finish(),
            Self::Provided { salt, nonce, manager } => f
                .debug_struct("Provided")
                .field("salt", salt)
                .field("nonce", nonce)
                .field("manager", &manager.is_some())
                .finish(),
        }
    }
}
