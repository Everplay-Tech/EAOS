...
class ThrorNN:
    def __init__(self):
        self.weights1 = np.array([[3.0 * np.sqrt(2), 3.1 * np.sqrt(2), 3.2 * np.sqrt(2), 3.3 * np.sqrt(2), 3.4 * np.sqrt(2)]])
        self.bias1 = np.array([3.0 * np.sqrt(2), 3.1 * np.sqrt(2), 3.2 * np.sqrt(2), 3.3 * np.sqrt(2), 3.4 * np.sqrt(2)])
        self.weights2 = np.array([[3.3 * np.sqrt(2), 3.4 * np.sqrt(2), 3.5 * np.sqrt(2), 3.6 * np.sqrt(2), 3.7 * np.sqrt(2)]])
        self.bias2 = np.array([3.0 * np.sqrt(2)])
    ...
    def update(self, actual, predicted, features):
        ...
        hidden = self.sigmoid(np.dot(features, self.weights1.T) + self.bias1)
        grad2 = error * hidden
        self.velocity2 = 0.9 * self.velocity2 - self.learning_rate * grad2
