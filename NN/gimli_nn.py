...
class GimliNN:
    def __init__(self):
        self.weights1 = np.array([[9.0 * np.sqrt(2), 9.1 * np.sqrt(2)], [9.2 * np.sqrt(2), 9.3 * np.sqrt(2)], [9.4 * np.sqrt(2), 9.5 * np.sqrt(2)], [9.6 * np.sqrt(2), 9.7 * np.sqrt(2)]])
        self.bias1 = np.array([9.0 * np.sqrt(2), 9.1 * np.sqrt(2), 9.2 * np.sqrt(2), 9.3 * np.sqrt(2)])
        self.weights2 = np.array([[9.3 * np.sqrt(2), 9.4 * np.sqrt(2), 9.5 * np.sqrt(2), 9.6 * np.sqrt(2)]])
        self.bias2 = np.array([9.0 * np.sqrt(2)])
...
if __name__ == "__main__":
    model = GimliNN()
    features = np.array([9.0, 9.1])
    pred = model.predict(features)
    print(f"Initial Prediction: {pred}")
    model.update(10.5, pred, features)
