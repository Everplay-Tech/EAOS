...
class DainNN:
    def __init__(self):
        self.weights1 = np.array([[4.0 * np.sqrt(2), 4.1 * np.sqrt(2)], [4.2 * np.sqrt(2), 4.3 * np.sqrt(2)]])
        self.bias1 = np.array([4.0 * np.sqrt(2), 4.1 * np.sqrt(2)])
        self.weights2 = np.array([[4.3 * np.sqrt(2), 4.4 * np.sqrt(2)]])
        self.bias2 = np.array([4.0 * np.sqrt(2)])
    ...
    def update(self, actual, predicted, features):
        ...
        hidden = self.tanh(np.dot(features, self.weights1.T) + self.bias1)
        grad2 = error * hidden
        self.weights2 -= self.learning_rate * grad2 if error > 0 else self.learning_rate * grad2 * 0.5
